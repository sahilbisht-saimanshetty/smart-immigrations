...
const WhySucceed = () => {
  return (
    <div
      id="whyChooseUs"
      className="relative w-full pb-16 md:pb-32 lg:pb-44 pt-10 md:pt-24 overflow-hidden"
    >
      ...
      <div className="space-y-8 md:space-y-10 lg:space-y-20 mt-10 md:mt-10 lg:mt-24">
        <Marquee direction="left" items={strategies} speed={85} />

        <Marquee direction="right" items={strategies} speed={85} />
      </div>
    </div>
  );
};
...
//         animate={{
//           x: direction === "right" ? ["0", "-100%"] : ["-97%", "0%"], // Adjust for both directions
//         }}
//         style={{
...
const Marquee = ({ direction = "left", items, speed = 100 }) => {
  ...
  return (
    <div className="relative w-full overflow-hidden">
      <div
        ...
        style={{
          animationDuration: `${speed}s`, // always add unit!
        }}
      >
        {duplicatedItems.map((item, index) => (
          <div key={`${item}-${index}`} className="marquee-item">
